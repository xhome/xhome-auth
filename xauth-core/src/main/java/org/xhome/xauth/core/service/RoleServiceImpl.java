...
public class RoleServiceImpl implements RoleService {
	...
	public int addRole(User oper, Role role) {
		...
		if (roleDAO.isRoleExists(role)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to add role {}, but it's already exists",
						name);
			}
	...
	public int updateRole(User oper, Role role) {
		...
		if (!this.beforeRoleManage(oper, Action.UPDATE, role)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update role {}[{}], but it's blocked",
						name, id);
			}
		...
		if (old == null) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update role {}[{}], but it's not exists",
						name, id);
			}
		...
		if (!old.getVersion().equals(role.getVersion())) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to update role {}[{}], but version not match",
						oldName, id);
			}

			this.logManage(oldName, Action.UPDATE, id,
					Status.VERSION_NOT_MATCH, oper);
			this.afterRoleManage(oper, Action.UPDATE, Status.VERSION_NOT_MATCH,
					role);
		...
		if (status == Status.NO_UPDATE || status == Status.LOCK) {
			if (logger.isDebugEnabled()) {
				logger.debug("it's not allowed to update role {}[{}]", oldName,
						id);
			}
		...
		if (!oldName.equals(name) && roleDAO.isRoleExists(role)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to update role {}[{}] to {}, but it's exists",
						oldName, id, name);
			}
	...
	public int lockRole(User oper, Role role) {
		...
		if (!this.beforeRoleManage(oper, Action.LOCK, role)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to lock role {}[{}], but it's blocked", name,
						id);
			}
	...
	public int unlockRole(User oper, Role role) {
		...
		if (!this.beforeRoleManage(oper, Action.UNLOCK, role)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to unlock role {}[{}], but it's blocked",
						name, id);
			}
	...
	public int deleteRole(User oper, Role role) {
		...
		if (!this.beforeRoleManage(oper, Action.DELETE, role)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to delete role {}[{}], but it's blocked",
						name, id);
			}
	...
	public int deleteRoles(User oper, List<Role> roles) {
		...
		for (Role role : roles) {
			...
			if (r != Status.SUCCESS) {
				throw new RuntimeException("fail to delete role ["
	...
	public boolean isRoleExists(User oper, Role role) {
		...
		if (!this.beforeRoleManage(oper, Action.IS_EXISTS, role)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge exists of role {}, but it's blocked",
						name);
			}
		...
		this.logManage(name, Action.IS_EXISTS, role.getId(), Status.SUCCESS,
				oper);
	...
	public boolean isRoleUpdateable(User oper, Role role) {
		...
		if (!this.beforeRoleManage(oper, Action.IS_UPDATEABLE, role)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to juge updateable of role {}[{}], but it's blocked",
						name, id);
			}

			this.logManage(name, Action.IS_UPDATEABLE, null, Status.BLOCKED,
					oper);
			this.afterRoleManage(oper, Action.IS_UPDATEABLE, Status.BLOCKED,
					role);
			return false;
		}
	...
	public boolean isRoleLocked(User oper, Role role) {
		...
		if (!this.beforeRoleManage(oper, Action.IS_LOCKED, role)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to juge locked of role {}[{}], but it's blocked",
						name, id);
			}
	...
	public boolean isRoleDeleteable(User oper, Role role) {
		...
		if (!this.beforeRoleManage(oper, Action.IS_DELETEABLE, role)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to juge deleteable of role {}[{}], but it's blocked",
						name, id);
			}

			this.logManage(name, Action.IS_DELETEABLE, null, Status.BLOCKED,
					oper);
			this.afterRoleManage(oper, Action.IS_DELETEABLE, Status.BLOCKED,
					role);
			return false;
		}
	...
	public List<Role> getRoles(User oper, QueryBase query) {
		if (!this.beforeRoleManage(oper, Action.QUERY, null, query)) {
			...
			this.afterRoleManage(oper, Action.QUERY, Status.BLOCKED, null,
					query);
			return null;
		}
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug("query roles with parameters {}",
						query.getParameters());
			} else {
	...
	public long countRoles(User oper, QueryBase query) {
		if (!this.beforeRoleManage(oper, Action.COUNT, null, query)) {
			...
			this.afterRoleManage(oper, Action.COUNT, Status.BLOCKED, null,
					query);
			return -1;
		}
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug("count roles with parameters {} of {}",
	...
	private void logManage(String content, Short action, Long obj,
		...
		ManageLog manageLog = new ManageLog(ManageLog.MANAGE_LOG_XAUTH,
				content, action, ManageLog.TYPE_ROLE, obj, oper == null ? null
						: oper.getId());
		manageLog.setStatus(status);
		manageLogService.logManage(manageLog);
	}

	private boolean beforeRoleManage(User oper, short action, Role role,
			Object... args) {
	...
	private void afterRoleManage(User oper, short action, short result,
	...
	public void setRoleManageListeners(
			List<RoleManageListener> roleManageListeners) {
