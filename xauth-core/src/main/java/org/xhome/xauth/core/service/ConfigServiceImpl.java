...
public class ConfigServiceImpl implements ConfigService {
    ...
    public int updateConfig(User oper, Config config) {
        ...
        if (old == null) {
            if (logger.isDebugEnabled()) {
                logger.debug("try to update config {}[{}], but it's not exists",
                                item, id);
            }
        ...
        if (!old.getVersion().equals(config.getVersion())) {
            if (logger.isDebugEnabled()) {
                logger.debug("try to update config {}[{}], but version not match",
                                oldName, id);
            }
        ...
        if (!oldName.equals(item) && configDAO.isConfigExists(config)) {
            if (logger.isDebugEnabled()) {
                logger.debug("try to update config {}[{}] to {}, but it's exists",
                                oldName, id, item);
            }
    ...
    public boolean isConfigExists(User oper, Config config) {
        ...
        if (!this.beforeConfigManage(oper, Action.IS_EXISTS, config)) {
            if (logger.isDebugEnabled()) {
                logger.debug("try to juge exists of config {}, but it's blocked",
                                item);
            }
    ...
    public boolean isConfigUpdateable(User oper, Config config) {
        ...
        if (!this.beforeConfigManage(oper, Action.IS_UPDATEABLE, config)) {
            if (logger.isDebugEnabled()) {
                logger.debug("try to juge updateable of config {}[{}], but it's blocked",
                                item, id);
            }
    ...
    public String getBaseURL() {
        ...
        String url = config != null ? config.getValue() : null;
