...
public class ConfigServiceImpl implements ConfigService {
	...
	public int addConfig(User oper, Config config) {
		...
		if (configDAO.isConfigExists(config)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to add config {}, but it's already exists", item);
			}
		...
		short r = configDAO.addConfig(config) == 1 ? Status.SUCCESS : Status.ERROR;
	...
	public int updateConfig(User oper, Config config) {
		...
		if (!this.beforeConfigManage(oper, Action.UPDATE, config)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update config {}[{}], but it's blocked", item, id);
			}
		...
		if (old == null) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update config {}[{}], but it's not exists", item, id);
			}
			
			this.logManage(item, Action.UPDATE, id, Status.NOT_EXISTS, oper);
			this.afterConfigManage(oper, Action.UPDATE, Status.NOT_EXISTS, config);
		...
		if (!old.getVersion().equals(config.getVersion())) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update config {}[{}], but version not match", oldName, id);
			}
			
			this.logManage(oldName, Action.UPDATE, id, Status.VERSION_NOT_MATCH, oper);
			this.afterConfigManage(oper, Action.UPDATE, Status.VERSION_NOT_MATCH, config);
		...
		if (status == Status.NO_UPDATE || status == Status.LOCK) {
			if (logger.isDebugEnabled()) {
				logger.debug("it's not allowed to update config {}[{}]", oldName, id);
			}
		...
		if (!oldName.equals(item) && configDAO.isConfigExists(config)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update config {}[{}] to {}, but it's exists", oldName, id, item);
			}
		...
		short r  = configDAO.updateConfig(config) == 1 ? Status.SUCCESS : Status.ERROR;
		if (r == Status.SUCCESS) {
	...
	public int lockConfig(User oper, Config config) {
		...
		if (!this.beforeConfigManage(oper, Action.LOCK, config)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to lock config {}[{}], but it's blocked", item, id);
			}
		...
		short r = configDAO.lockConfig(config) == 1 ? Status.SUCCESS : Status.ERROR;
	...
	public int unlockConfig(User oper, Config config) {
		...
		if (!this.beforeConfigManage(oper, Action.UNLOCK, config)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to unlock config {}[{}], but it's blocked", item, id);
			}
		...
		short r = configDAO.unlockConfig(config) == 1 ? Status.SUCCESS : Status.ERROR;
	...
	public int removeConfig(User oper, Config config) {
		...
		if (!this.beforeConfigManage(oper, Action.REMOVE, config)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to remove config {}[{}], but it's blocked", item, id);
			}
	...
	public int deleteConfig(User oper, Config config) {
		...
		if (!this.beforeConfigManage(oper, Action.DELETE, config)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to delete config {}[{}], but it's blocked", item, id);
			}
	...
	public boolean isConfigExists(User oper, Config config) {
		...
		if (!this.beforeConfigManage(oper, Action.IS_EXISTS, config)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge exists of config {}, but it's blocked", item);
			}
			
			this.logManage(item, Action.IS_EXISTS, null, Status.BLOCKED, oper);
			this.afterConfigManage(oper, Action.IS_EXISTS, Status.BLOCKED, config);
			return false;
		}
		...
		this.logManage(item, Action.IS_EXISTS, config.getId(), Status.SUCCESS, oper);
	...
	public boolean isConfigUpdateable(User oper, Config config) {
		...
		if (!this.beforeConfigManage(oper, Action.IS_UPDATEABLE, config)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge updateable of config {}[{}], but it's blocked", item, id);
			}
			
			this.logManage(item, Action.IS_UPDATEABLE, null, Status.BLOCKED, oper);
			this.afterConfigManage(oper, Action.IS_UPDATEABLE, Status.BLOCKED, config);
			return false;
		}
		...
		this.afterConfigManage(oper, Action.IS_UPDATEABLE, Status.SUCCESS, config);
		return e;
	}
	...
	public boolean isConfigLocked(User oper, Config config) {
		...
		if (!this.beforeConfigManage(oper, Action.IS_LOCKED, config)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge locked of config {}[{}], but it's blocked", item, id);
			}
			
			this.logManage(item, Action.IS_LOCKED, null, Status.BLOCKED, oper);
			this.afterConfigManage(oper, Action.IS_LOCKED, Status.BLOCKED, config);
			return false;
		}
	...
	public boolean isConfigRemoveable(User oper, Config config) {
		...
		if (!this.beforeConfigManage(oper, Action.IS_REMOVEABLE, config)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge removeable of config {}[{}], but it's blocked", item, id);
			}
			
			this.logManage(item, Action.IS_REMOVEABLE, null, Status.BLOCKED, oper);
			this.afterConfigManage(oper, Action.IS_REMOVEABLE, Status.BLOCKED, config);
			return false;
		}
		...
		this.afterConfigManage(oper, Action.IS_REMOVEABLE, Status.SUCCESS, config);
		return e;
	}
	...
	public boolean isConfigDeleteable(User oper, Config config) {
		...
		if (!this.beforeConfigManage(oper, Action.IS_DELETEABLE, config)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge deleteable of config {}[{}], but it's blocked", item, id);
			}
			
			this.logManage(item, Action.IS_DELETEABLE, null, Status.BLOCKED, oper);
			this.afterConfigManage(oper, Action.IS_DELETEABLE, Status.BLOCKED, config);
			return false;
		}
		...
		this.afterConfigManage(oper, Action.IS_DELETEABLE, Status.SUCCESS, config);
		return e;
	}
	...
	public List<Config> getConfigs(User oper, QueryBase query) {
		if (!this.beforeConfigManage(oper, Action.QUERY, null, query)) {
			...
			this.afterConfigManage(oper, Action.QUERY, Status.BLOCKED, null, query);
			return null;
		}
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug("query configs with parameters {}", query.getParameters());
			} else {
	...
	public long countConfigs(User oper, QueryBase query) {
		if (!this.beforeConfigManage(oper, Action.COUNT, null, query)) {
			...
			this.afterConfigManage(oper, Action.COUNT, Status.BLOCKED, null, query);
			return -1;
		}
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug("count configs with parameters {} of {}", query.getParameters(), c);
			} else {
	...
	private void logManage(String content, Short action, Long obj, Short status, User oper) {
		ManageLog manageLog = new ManageLog(ManageLog.MANAGE_LOG_XAUTH, content, action, ManageLog.TYPE_CONFIG, obj, oper == null ? null : oper.getId());
		manageLog.setStatus(status);
		manageLogService.logManage(manageLog);
	}
	
	private boolean beforeConfigManage(User oper, short action, Config config, Object ...args) {
	...
	private void afterConfigManage(User oper, short action, short result, Config config, Object ...args) {
		if (configManageListeners != null) {
			for (ConfigManageListener listener : configManageListeners) {
	...
	public void setConfigManageListeners(List<ConfigManageListener> configManageListeners) {
	...
	public void registerConfigManageListener(ConfigManageListener configManageListener) {
		if (configManageListeners == null) {
