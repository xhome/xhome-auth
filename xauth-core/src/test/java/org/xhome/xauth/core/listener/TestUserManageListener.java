...
public class TestUserManageListener implements UserManageListener {
	...
	private Logger logger = LoggerFactory.getLogger(TestUserManageListener.class);
	
	@Override
	public boolean beforeUserManage(User oper, short action, User user,
			Object... args) {
		logger.debug("TEST BEFORE USER MANAGE LISTENER {} {} {}", oper.getName(), action, user != null ? user.getName() : "NULL");
		return true;
	}
	...
	public void afterUserManage(User oper, short action, short result,
		...
		logger.debug("TEST AFTER USER MANAGE LISTENER {} {} {}", oper.getName(), action, user != null ? user.getName() : "NULL");
	}
