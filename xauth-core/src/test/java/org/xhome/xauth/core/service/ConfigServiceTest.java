...
public class ConfigServiceTest extends AbstractTest {
	...
	public ConfigServiceTest() {
		...
		((ConfigServiceImpl)configService).registerConfigManageListener(new TestConfigManageListener());
	}
	...
	public void testCountConfigs() {
		logger.info("{}", configService.countConfigs(oper));
	...
	public void testGetConfigs() {
		List<Config> configs = configService.getConfigs(oper);
		printConfig(configs);
		
		QueryBase query = new QueryBase();
		query.addParameter("name", "test");
		configs = configService.getConfigs(oper, query);
		printConfig(configs);
	}
	...
	public void testIsConfigExists() {
		logger.info("{}", configService.isConfigExists(oper, new Config("common_test", "abc")));
	}
