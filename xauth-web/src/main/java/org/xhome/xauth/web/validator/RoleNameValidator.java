	...
	public boolean validate(Object target, Errors errors) {
		...
		if (StringUtils.isEmpty(name)) {
			errors.rejectValue(FIELD_NAME, CODE_NAME_EMPTY, validationConfig
					.getConfig(AuthValidatorConfig.ROLE_NAME_EMPTY_MESSAGE));
		} else {
			int size = name.length();
			int min = Integer.parseInt(validationConfig
					.getConfig(AuthValidatorConfig.ROLE_NAME_SIZE_MIN)), max = Integer
					.parseInt(validationConfig
							.getConfig(AuthValidatorConfig.ROLE_NAME_SIZE_MAX));
			if (size < min || size > max) {
				errors.rejectValue(FIELD_NAME, CODE_NAME_SIZE, validationConfig
						.getConfig(AuthValidatorConfig.ROLE_NAME_SIZE_MESSAGE));
			} else {
				if (!name
						.matches(validationConfig
								.getConfig(AuthValidatorConfig.ROLE_NAME_PATTERN_REGEXP))) {
					errors.rejectValue(
							FIELD_NAME,
							CODE_NAME_PATTERN,
							validationConfig
									.getConfig(AuthValidatorConfig.ROLE_NAME_PATTERN_MESSAGE));
				} else {
					return true;
				}
			}
