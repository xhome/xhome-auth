...
public class UserNickValidator extends Validator {
	...
	public boolean validate(Object target, Errors errors) {
		...
		if (StringUtils.isEmpty(nick)) {
			errors.rejectValue(FIELD_NICK, CODE_NICK_EMPTY, validationConfig.getConfig(AuthValidatorConfig.USER_NICK_EMPTY_MESSAGE));
		} else {
			int size = nick.length();
			int min = Integer.parseInt(validationConfig.getConfig(AuthValidatorConfig.USER_NICK_SIZE_MIN)), 
				max = Integer.parseInt(validationConfig.getConfig(AuthValidatorConfig.USER_NICK_SIZE_MAX));
			if (size < min || size > max) {
				errors.rejectValue(FIELD_NICK, CODE_NICK_SIZE, validationConfig.getConfig(AuthValidatorConfig.USER_NICK_SIZE_MESSAGE));
			} else {
				return true;
			}
		}
