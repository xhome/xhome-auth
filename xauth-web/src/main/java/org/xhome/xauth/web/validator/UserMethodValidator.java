	...
	public boolean validate(Object target, Errors errors) {
		...
		if (StringUtils.isNotEmpty(method)) {
			...
			int min = Integer.parseInt(validationConfig
					.getConfig(AuthValidatorConfig.USER_METHOD_SIZE_MIN)), max = Integer
					.parseInt(validationConfig
							.getConfig(AuthValidatorConfig.USER_METHOD_SIZE_MAX));
			if (size < min || size > max) {
				errors.rejectValue(
						FIELD_METHOD,
						CODE_METHOD_SIZE,
						validationConfig
								.getConfig(AuthValidatorConfig.USER_METHOD_SIZE_MESSAGE));
			} else {
				return true;
			}
		}
