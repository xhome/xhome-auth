...
public class UserRolesValidator extends Validator {
	...
	public boolean validate(Object target, Errors errors) {
		...
		if (roles == null || roles.isEmpty()) {
			errors.rejectValue(FIELD_ROLES, CODE_ROLES_EMPTY, validationConfig.getConfig(AuthValidatorConfig.USER_ROLES_EMPTY_MESSAGE));
			return false;
		} else {
			for (Role role : roles) {
				if (idValidator.validate(role, errors) && roleNameValidator.validate(role, errors)) {
					continue;
				}
				return false;
			}
